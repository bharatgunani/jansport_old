...
class Payment extends \Magento\Payment\Model\Method\Cc
{
    ...
    // ->get('Psr\Log\LoggerInterface')->debug(json_encode($data->getData()));
		 // die;
        $info = $this->getInfoInstance();

		
        $info->setCcType(
            $data->getData()['cc_type']
        )->setCcOwner(
            $data->getData()['cc_ownerx']
        )->setCcLast4(
            substr($data->getData()['cc_number'], -4)
        )->setCcNumber(
            $data->getData()['cc_number']
        )->setCcCid(
            $data->getData()['cc_cid']
        )->setCcExpMonth(
            $data->getData()['cc_exp_month']
        )->setCcExpYear(
            $data->getData()['cc_exp_year']
        )->setCcSsIssue(
            $data->getData()['cc_ss_issue']
        )->setCcSsStartMonth(
            $data->getData()['cc_ss_start_month']
        )->setCcSsStartYear(
            $data->getData()['cc_ss_start_year']
        );
        return $this;
    }
}