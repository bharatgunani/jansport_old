...
namespace Navin\Bestseller\Block;
...
class AbstractSlider extends \Magento\Catalog\Block\Product\AbstractProduct implements \Magento\Widget\Block\BlockInterface
{
	...
	public $productCacheKey = 'navin_product_slider_cache';
	...
	public function getAllOptions()
	{
		...
		foreach ($this->_allOptions as $key => $option) {
			if(($key == 'responsive') && ($option)) { //$option is positive: 1, 2, true (boolen), "true"
				$objectManager  = \Magento\Framework\App\ObjectManager::getInstance();
				$helperConfig = $objectManager->create('Navin\Bestseller\Helper\Data');
				$sliderOptions = $sliderOptions.$key.':'.'{ 0:{ items:'.$helperConfig->getSliderConfig(\Navin\Bestseller\Helper\Data::NUMBER_ITEMS_SLIDER_SCREEN_SIZE_0).'},'
					.'600:{ items:'.$helperConfig->getSliderConfig(\Navin\Bestseller\Helper\Data::NUMBER_ITEMS_SLIDER_SCREEN_SIZE_600).'},'
					.'1000:{ items:'.$helperConfig->getSliderConfig(\Navin\Bestseller\Helper\Data::NUMBER_ITEMS_SLIDER_SCREEN_SIZE_1000).'}},';
			}else {
	...
	public function getAllConfigValues()
	{
		 ...
		 */
		$objectManager  = \Magento\Framework\App\ObjectManager::getInstance();
		$adminConfig = $objectManager->create('Navin\Bestseller\Helper\Data')->getSliderConfig('bestsellerting_setting/slider');
		return $adminConfig;
	}
