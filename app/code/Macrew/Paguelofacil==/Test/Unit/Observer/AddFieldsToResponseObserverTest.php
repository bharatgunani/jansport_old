 ...
 */
namespace Macrew\Paguelofacil\Test\Unit\Observer;
...
class AddFieldsToResponseObserverTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * @var \Macrew\Paguelofacil\Model\Directpost|\PHPUnit_Framework_MockObject_MockObject
     */
    protected $paymentMock;

    /**
     * @var \Macrew\Paguelofacil\Model\Directpost\Session|\PHPUnit_Framework_MockObject_MockObject
     */
    ...
    /**
     * @var \Macrew\Paguelofacil\Observer\AddFieldsToResponseObserver
     */
    ...
    protected function setUp()
    {
        ...
        $this->coreRegistryMock = $this->getMockBuilder('Magento\Framework\Registry')
            ...
            ->getMock();
        $this->paymentMock = $this->getMockBuilder('Macrew\Paguelofacil\Model\Directpost')
            ->disableOriginalConstructor()
            ->getMock();
        $this->sessionMock = $this->getMockBuilder('Macrew\Paguelofacil\Model\Directpost\Session')
            ->disableOriginalConstructor()
        ...
        $this->addFieldsToResponseObserver = $helper->getObject(
            'Macrew\Paguelofacil\Observer\AddFieldsToResponseObserver',
            [
    ...
    public function testAddFieldsToResponseSuccess()
    {
        ...
        $orderPaymentMock = $this->getMockBuilder('Magento\Sales\Model\Order\Payment')
            ->disableOriginalConstructor()
            ->getMock();
        $instanceMock = $this->getMockBuilder('Macrew\Paguelofacil\Model\Directpost')
            ->disableOriginalConstructor()
            ->getMock();
        $requestToPaguelofacilMock = $this->getMockBuilder('Macrew\Paguelofacil\Model\Directpost\Request')
            ->disableOriginalConstructor()
        ...
        $instanceMock->expects($this->once())
            ...
            ->with($orderMock)
            ->willReturn($requestToPaguelofacilMock);
        ...
        $requestToPaguelofacilMock->expects($this->once())
            ->method('setControllerActionName')
            ->with($testData['requestToPaguelofacil.setControllerActionName']);
        ...
        $requestToPaguelofacilMock->expects($this->once())
            ->method('setIsSecure')
            ->with($testData['requestToPaguelofacil.setIsSecure']);
        $requestToPaguelofacilMock->expects($this->once())
            ->method('getData')
            ->willReturn($testData['requestToPaguelofacil.getData']);
        $this->resultMock->expects($this->once())
            ->method('setData')
    ...
    protected function getAddFieldsToResponseSuccessTestData()
    {
        ...
        return [
            ...
            'requestToPaguelofacil.setControllerActionName' => $controllerName,
            'store.isCurrentlySecure' => $secure,
            'requestToPaguelofacil.setIsSecure' => $secure,
            'requestToPaguelofacil.getData' => $requestFields,
            'result.setData' => [
                'error' => false,
                'test-payment-code' => [
