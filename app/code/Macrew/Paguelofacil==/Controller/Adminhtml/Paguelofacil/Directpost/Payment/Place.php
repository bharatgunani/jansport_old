 ...
 */
namespace Macrew\Paguelofacil\Controller\Adminhtml\Paguelofacil\Directpost\Payment;
...
use Macrew\Paguelofacil\Helper\Backend\Data as DataHelper;
        ...
        if (isset($paymentParam['method'])) {
            ...
            try {
                ...
                if ($payment && $payment->getMethod() == $this->_objectManager->create(
                    ...
                    $session = $this->_objectManager->get('Macrew\Paguelofacil\Model\Directpost\Session');
                    $session->addCheckoutOrderIncrementId($order->getIncrementId());
                    $session->setLastOrderIncrementId($order->getIncrementId());

                    /** @var \Macrew\Paguelofacil\Model\Directpost $method */
                    $method = $payment->getMethodInstance();
                    $method->setDataHelper($this->helper);
                    $requestToPaguelofacil = $method->generateRequestFromOrder($order);
                    $requestToPaguelofacil->setControllerActionName($controller);
                    $requestToPaguelofacil->setOrderSendConfirmation($sendConfirmationFlag);
                    $requestToPaguelofacil->setStoreId($this->_getOrderCreateModel()->getQuote()->getStoreId());

                    $adminUrl = $this->_objectManager->get('Magento\Backend\Model\UrlInterface');
                    if ($adminUrl->useSecretKey()) {
                        $requestToPaguelofacil->setKey(
                            $adminUrl->getSecretKey('adminhtml', 'paguelofacil_directpost_payment', 'redirect')
                        );
                    }
                    $result['directpost'] = ['fields' => $requestToPaguelofacil->getData()];
                }
