...
return!(pass.length>0&&pass.length<6);},'Please enter 6 or more characters. Leading or trailing spaces will be ignored.'],"validate-admin-password":[function(v){if(v==null){return false;}
var pass=$.trim(v);if(0===pass.length){return true;}
if(!(/[a-z]/i.test(v))||!(/[0-9]/.test(v))){return false;}
if(pass.length<7){return false;}
return true;},'Please enter 7 or more characters, using both numeric and alphabetic.'],"validate-url":[function(v){if($.mage.isEmptyNoTrim(v)){return true;}
...
if(!valid)
valid=!this.optional(element);$('input['+params.dataSku+'=true]').each(function(){if($(this).val()!==''){valid=true;}});return valid;},'Enter valid SKU key'],'required-if-specified':[function(value,element,params){var valid=true;var dependent=$(params);if(dependent.length>0){valid=this.check(dependent);if(valid){var dependentValue=dependent.val();valid=typeof dependentValue!='undefined'&&dependentValue.length>0;}}
