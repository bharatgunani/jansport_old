...
var headElement=$(idTo+'-head'),inputs=$(idTo).up(this._config.levels_up).select('input','select','td'),isInheritCheckboxChecked=$(idTo+'_inherit')&&$(idTo+'_inherit').checked,isAnInputOrSelect=['input','select'].indexOf($(idTo).tagName.toLowerCase())!=-1;if(shouldShowUp){var currentConfig=this._config;if(inputs){inputs.each(function(item){if((!item.type||item.type!='hidden')&&!($(item.id+'_inherit')&&$(item.id+'_inherit').checked)&&!(currentConfig.can_edit_price!=undefined&&!currentConfig.can_edit_price)){item.disabled=false;jQuery(item).removeClass('ignore-validate');}});}
if(headElement){headElement.show();if(headElement.hasClassName('open')){$(idTo).show();}else{$(idTo).hide();}}else{$(idTo).show();if(isAnInputOrSelect&&!isInheritCheckboxChecked){$(idTo).disabled=false;jQuery('#'+idTo).removeClass('ignore-validate');}}}else{if(inputs){inputs.each(function(item){if((!item.type||item.type!='hidden')&&!($(item.id+'_inherit')&&$(item.id+'_inherit').checked)){item.disabled=true;jQuery(item).addClass('ignore-validate');}});}
if(headElement){headElement.hide();}
$(idTo).hide();if(isAnInputOrSelect&&!isInheritCheckboxChecked){$(idTo).disabled=true;jQuery('#'+idTo).addClass('ignore-validate');}}
var rowElement=$('row_'+idTo);if(rowElement==undefined){rowElement=$(idTo).up(this._config.levels_up);}
