...
return;}}};var _showPopup=function(dialog,title,okButton,url){$.ajax({url:url,cache:false,data:{form_key:window.FORM_KEY},method:'GET',beforeSend:function(){$('body').trigger('processStart');},success:function(result){var redirect=result._redirect;if(redirect){window.location.href=redirect;return;}
var identityLinkUrl=null,consumerId=null,popupHtml=null,popup=$('#integration-popup-container');try{var resultObj=typeof result==='string'?JSON.parse(result):result;identityLinkUrl=resultObj['identity_link_url'];consumerId=resultObj['consumer_id'];popupHtml=resultObj['popup_content'];}catch(e){}
if(identityLinkUrl&&consumerId&&popupHtml){IdentityLogin.invokePopup(identityLinkUrl,consumerId,popup);}else{popupHtml=result;}
